...
interface RetellWidgetProps { lang: Language; }
...
export default function RetellWidget({ lang }: RetellWidgetProps) {
  ...
  useEffect(() => {
    const timer = setTimeout(() => { if (status === 'idle') setShowPromo(true); }, 8000);
  ...
  useEffect(() => {
    ...
    retellClient.on('update', (update: { transcript?: { role: string; content: string }[] }) => {
      if (update.transcript) setTranscript(update.transcript.map(t => ({ role: t.role as 'agent' | 'user', content: t.content })));
    });
    retellClient.on('error', () => {
      setStatus('error');
      setErrorMsg(lang === 'TR' ? 'Baglanti hatasi. Lutfen tekrar deneyin.' : 'Connection error. Please try again.');
    });
    return () => { retellClient.removeAllListeners(); };
  }, [lang]);

  async function startCall() {
    setStatus('connecting'); setErrorMsg(''); setTranscript([]); setShowPostCall(false);
    try {
      ...
      if (!res.ok) { const d = await res.json(); throw new Error(d.error ?? 'Sunucu hatasi'); }
      const { access_token } = await res.json();
      await retellClient.startCall({ accessToken: access_token, sampleRate: 24000 });
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : 'Bilinmeyen hata';
      setStatus('error');
      setErrorMsg(msg.toLowerCase().includes('permission') ? (lang === 'TR' ? 'Mikrofon izni reddedildi.' : 'Mic permission denied.') : msg);
    }
  }
    ...
    <>
      {showPromo && status === 'idle' && (
        <div className="fixed bottom-28 right-6 z-40 bg-white text-black rounded-2xl shadow-2xl p-4 max-w-xs border border-black/10">
          <button onClick={() => setShowPromo(false)} className="absolute top-2 right-3 text-black/40 hover:text-black text-lg leading-none">x</button>
          <p className="text-sm font-medium pr-4">{lang === 'TR' ? 'AI asistanimizi sesli olarak deneyin.' : 'Try our AI assistant with your voice.'}</p>
          <button onClick={() => { setShowPromo(false); startCall(); }} className="mt-2 text-xs underline underline-offset-2 text-black/60 hover:text-black">
      ...
      {showPostCall && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-white text-black rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center">
            ...
            <h3 className="text-lg font-semibold mb-2">{lang === 'TR' ? 'Nasil buldunuz?' : 'How was it?'}</h3>
            <p className="text-sm text-black/60 mb-6">
              {lang === 'TR' ? 'Klinginize ozel bu sistemi 15 dakikalik gorusmede anlatalim.' : 'Let us show you this for your clinic in a 15-minute call.'}
            </p>
            <button
              onClick={openCal}
            ...
            <button onClick={() => setShowPostCall(false)} className="text-xs text-black/40 hover:text-black/70 transition-colors">
      ...
      {status === 'connected' && (
        <div className="fixed bottom-24 right-6 z-40 bg-white text-black rounded-2xl shadow-2xl border border-black/10 w-80 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-black/10">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium">
                {agentTalking ? (lang === 'TR' ? 'Vera konusuyor...' : 'Vera is talking...') : (lang === 'TR' ? 'Vera dinliyor...' : 'Vera is listening...')}
              </span>
            </div>
            <div className="flex items-center gap-1">
              <span className="w-1.5 h-1.5 rounded-full bg-black animate-ping" />
              <span className="text-xs text-black/50 uppercase tracking-wide">{lang === 'TR' ? 'Canli' : 'Live'}</span>
            </div>
          </div>
            ...
            <div ref={transcriptRef} className="px-4 py-3 max-h-40 overflow-y-auto space-y-2">
              {recentTranscript.map((line, i) => (
                <div key={i} className={`flex ${line.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <span className={`text-xs px-3 py-1.5 rounded-xl max-w-xs leading-relaxed ${line.role === 'user' ? 'bg-black text-white' : 'bg-black/10 text-black'}`}>
                    {line.content}
                  </span>
                </div>
              ))}
          ...
          <div className="px-4 py-3 border-t border-black/10">
            <button onClick={stopCall} className="w-full bg-black text-white text-sm font-medium py-2.5 rounded-xl hover:bg-black/80 transition-colors">
      ...
      <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-2">
        {status === 'error' && errorMsg && (
          <div className="bg-white text-black text-xs px-3 py-2 rounded-xl shadow-lg border border-black/10 max-w-56 text-right">{errorMsg}</div>
        )}
        <button
          ...
          className={`flex items-center gap-2 px-5 py-3 rounded-2xl font-medium text-sm shadow-xl transition-all duration-200 ${
            status === 'connecting' ? 'bg-black/70 text-white cursor-wait' :
            status === 'connected' ? 'bg-white text-black border border-black/20 hover:bg-black/5' :
